...
export class FileController {
  ...
  async uploadFile(@Req() req: Request, @Res() res: Response) {
    try {
      if (!req.fileHandler) {
        return res.status(400).json({ message: 'No file handler found' });
      }
      ...
      await new Promise((resolve, reject) => {
        ...
        busboy.on('file', async (fieldname: string, file: Readable, info: any) => {
          try {
            await req.fileHandler(fieldname, file, info.filename);
            resolve(null);
          } catch (error) {
            reject(error);
          }
...
1. 上传进度监控:
   ...
   export class FileUploadMiddleware implements NestMiddleware {
     ...
     async use(req: Request, res: Response, next: NextFunction) {
       ...
       try {
         ...
         // 创建进度监控流
         const createProgressStream = (totalSize: number) => {
           return new Transform({
             transform: (chunk: Buffer, encoding: string, callback: Function) => {
               ...
               // 通过WebSocket发送进度信息
               this.server.emit('uploadProgress', {
                 fileId,
                 progress: progress.toFixed(2),
                 uploadedBytes,
                 totalSize,
                 speed: this.calculateSpeed(chunk.length),
                 remainingTime: this.calculateRemainingTime(totalSize, uploadedBytes)
               });
         ...
         res.on('finish', () => {
           ...
           // 发送上传完成事件
           this.server.emit('uploadComplete', {
             fileId,
     ...
     private calculateSpeed(chunkSize: number): number {
       ...
       this.lastChunkTime = now;
       return chunkSize / (timeDiff / 1000);
     }
   
     // 计算剩余时间（秒）
     private calculateRemainingTime(total: number, uploaded: number): number {
       const remaining = total - uploaded;
       const speed = this.calculateSpeed(uploaded);
       return speed ? remaining / speed : 0;
     }
   ...
   export class UploadComponent {
     ...
     constructor() {
       ...
       this.socket.on('uploadProgress', (data: any) => {
         ...
         console.log(`Upload speed: ${this.formatSpeed(data.speed)}`);
         console.log(`Remaining time: ${this.formatTime(data.remainingTime)}`);
       });
   
       // 监听上传完成事件
       this.socket.on('uploadComplete', (data: any) => {
         console.log(`Upload ${data.success ? 'completed' : 'failed'} for file ${data.fileId}`);
       });
     }
   ...
   1. 实时进度监控：使用WebSocket实时发送上传进度信息
   2. 上传速度计算：计算并显示当前上传速度
   3. 剩余时间估算：基于当前速度估算剩余上传时间
   4. 错误处理：在上传过程中出现错误时及时通知
   5. 完成事件：上传完成后发送通知

   通过WebSocket，我们可以实时向客户端推送进度信息，提供更好的用户体验。进度信息包括：
