...
[观看 Vue School 的导航守卫如何工作的免费视频课程 (英文)](https://vueschool.io/lessons/how-to-configure-an-authentication-middleware-route-guard-with-vue-router?friend=vuejs)
...
`beforeRouteLeave`可以使用在用户有未提交内容时离开的提示

```js
export default {
    beforeRouteEnter(to, from, next) {
        // 此时不能访问this，因为守卫在导航确认前被调用，因此即将登场的新组件还没被创建。但是可以在回调中访问vm
        // to -> 去哪[path的路径为路由跳转后的路径]
        // from -> 从哪来[上一个路由的地址路径]
        // next ->下一步，必须要调用，不然不会跳转
