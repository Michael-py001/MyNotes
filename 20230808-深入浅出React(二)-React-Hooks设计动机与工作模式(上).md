# 20230808-深入浅出React-React-Hooks设计动机与工作模式(上)
...
![1.png](https://s2.loli.net/2023/08/08/uTWRn69vPDjc4pz.png)
...
更要命的是，由于开发者编写的逻辑在**封装**后是和组件粘在一起的，这就使得**类组件内部的逻辑难以实现拆分和复用。**如果你想要打破这个僵局，则需要进一步学习更加复杂的设计模式（比如高阶组件、Render Props 等），用更高的学习成本来交换一点点编码的灵活度。
...
> **函数式组件捕获了渲染所使用的值。，这是两类组件最大的不同。**

下面引用Dan的文章中的核心说明一下类组件和函数组件之间的区别和为什么这样做：

> ![image-20230808165256567](https://s2.loli.net/2023/08/08/MJqvgDTo5V4hHia.png)
>
...
![image-20230808160157617](https://s2.loli.net/2023/08/08/JyUBvgQmN7hk1Pj.png)
...
为什么类组件做不到？这里我摘出上述[文章](https://overreacted.io/how-are-function-components-different-from-classes/)中的 Demo，站在一个新的视角来解读一下“**函数式组件捕获了渲染所使用的值，这是两类组件最大的不同**”这个结论。首先我们来看这样一个类组件：
...
看起来好像没啥毛病，但是如果你在这个[在线 Demo](https://codesandbox.io/s/pjqnl16lm7)中尝试点击基于**类组件**形式编写的 Follow按钮后 3s 内把用户切换为 Sophie；弹窗的显示名字是Sophie。
...
这个问题就是：我们点击时follow的是Dan，但是提示却是follow了Sophie，这不是我们所期待的。
...
props 会在 ProfilePage 函数执行的一瞬间就被捕获，因为函数的每一次执行都会创建一个新的执行上下文，当handleClick执行时，里面的`showMessage`函数的内容`props.user`是当前执行的上下文中的`props`，由于setTimeout的原因这个上下文中的内容还在被引用，还未被释放回收，其实这也就是运用了**闭包**的特性。因此**我们可以充分确保从现在开始，在任何时机下读取到的 props，都是最初捕获到的那个 props**。

当父组件传入新的 props 来尝试重新渲染 ProfilePage 时，本质上是基于新的 props 入参发起了一次全新的函数调用，并不会影响上一次调用对上一个 props 的捕获。这样一来，我们便确保了渲染结果确实能够符合预期。

如果你认真阅读了我前面说过的那些话，相信你现在一定也不仅仅能够充分理解 Dan 所想要表达的“**函数式组件捕获了渲染所使用的值**”这个结论，而是能够更进一步地意识到这样一件事情：**函数组件真正地把数据和渲染绑定到了一起**。
