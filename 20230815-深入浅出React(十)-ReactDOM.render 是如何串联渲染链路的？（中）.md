...
![Drawing 0.png](https://s2.loli.net/2023/08/15/RPIJKpEz9iQGskd.png)
...
![Drawing 1.png](https://s2.loli.net/2023/08/15/HIVQSlfOqX5Jidb.png)
...
![Drawing 2.png](https://s2.loli.net/2023/08/15/edMIKWhZjBxHoy1.png)
...
![Drawing 3.png](https://s2.loli.net/2023/08/15/E5MBVlinQHWyCup.png)
...
![1.png](https://s2.loli.net/2023/08/15/zqEVL6YWcQ7o89u.png)
...
而 performUnitOfWork 函数将触发对 beginWork 的调用，进而实现对**新 Fiber 节点的创建**。若 beginWork 所创建的 Fiber 节点不为空，则 performUniOfWork 会用这个新的 Fiber 节点来**更新 workInProgress 的值**，**为下一次循环做准备**。

通过循环调用 performUnitOfWork 来触发 beginWork，新的 Fiber 节点就会被**不断地创建**。当 workInProgress 终于为空时，说明没有新的节点可以创建了，也就意味着已经完成对整棵 Fiber 树的构建。

在这个过程中，**每一个被创建出来的新 Fiber 节点，都会一个一个挂载为最初那个 workInProgress 节点（如下图高亮处）的后代节点。**而上述过程中构建出的这棵 Fiber 树，也正是大名鼎鼎的 **workInProgress 树**。

![2.png](https://s2.loli.net/2023/08/15/L8274TzstS5y9ej.png)
...
- beginWork 的入参是**一对用 alternate 连接起来的 workInProgress 和 current 节点**；

- beginWork 的核心逻辑是**根据 fiber 节点（workInProgress）的 tag 属性的不同，调用不同的节点创建函数**。

当前的 current 节点是 rootFiber，而 workInProgress 则是 current 的副本，它们的 tag 都是 3，如下图所示：

![Drawing 6.png](https://s2.loli.net/2023/08/17/258LAtqjeFHB9pK.png)
...
幸运的是，这些函数之间不仅命名形式一致，工作内容也相似。就 render 链路来说，它们共同的特性，就是都会通过**调用 reconcileChildren 方法，生成当前节点的子节点。**
...
![Drawing 7.png](https://s2.loli.net/2023/08/17/nZBkUmN7xSD4LtV.png)
...
![Drawing 8.png](https://s2.loli.net/2023/08/17/qijGIgbaBnLlxdV.png)
...
![Drawing 9.png](https://s2.loli.net/2023/08/17/CIbJcHk9vGDYZwj.png)

reconcileSingleElement 将基于 rootFiber 子节点的 ReactElement 对象信息，创建其对应的 FiberNode。这个过程中涉及的函数调用如下图高亮处所示：

![Drawing 10.png](https://s2.loli.net/2023/08/17/OyWME3H8phj65va.png)
...
![Drawing 11.png](https://s2.loli.net/2023/08/18/Rm5oQcY1D8E3qsO.png)
...
![Drawing 12.png](https://s2.loli.net/2023/08/18/OG6dW3TUBjzXlw7.png)

reconcileChildren 函数上下文里的 workInProgress 就是 rootFiber 节点。那么此时，我们就将新创建的 App Fiber 节点和 rootFiber 关联了起来，整个 Fiber 树如下图所示：

![3.png](https://s2.loli.net/2023/08/18/bvnulm4eJLPipDX.png)
...
![7.png](https://s2.loli.net/2023/08/18/SaeAs6C2cPMgujo.png)
...
![Drawing 15.png](https://s2.loli.net/2023/08/18/nfroMSyqdNjW7u6.png)
...
![Drawing 16.png](https://s2.loli.net/2023/08/18/RzfT6IM5NHFXhx3.png)

- class 为 container 的 DOM 元素对应的节点，其内容如下图所示
  ![Drawing 17.png](https://s2.loli.net/2023/08/18/4SxnD7msIBMquCL.png)

- h1 标签对应的节点
- 第 1 个 p 标签对应的 FiberNode，内容为“我是第一段话”，如下图所示
  ![Drawing 18.png](https://s2.loli.net/2023/08/18/SXnpxVycdvjhTuB.png)

- 第 2 个 p 标签对应的 FiberNode，内容为“我是第二段话”，如下图所示
  ![Drawing 19.png](https://s2.loli.net/2023/08/18/mdzyqHNU6tXoAu3.png)
...
![4.png](https://s2.loli.net/2023/08/18/oRsiFMq2auSJgUv.png)
...
![Drawing 21.png](https://s2.loli.net/2023/08/18/SDeYnfvJdZKqoU2.png)

return 属性局部截图：

![Drawing 22.png](https://s2.loli.net/2023/08/18/OfdSA2Fnlc9Km7Q.png)

sibling 属性局部截图：

![Drawing 23.png](https://s2.loli.net/2023/08/18/uj3ZApINr5vefai.png)
...
![5.png](https://s2.loli.net/2023/08/18/dyzROfpegLkBHo1.png)
